...
public class PlantsLab {
    ...
    private PlantsLab(Context context){
        ...
        SpeciesCreation speciesCreation = new SpeciesCreation();
    ...
    private static ContentValues getPlantsContentValues(Plant plant){
        ...
        values.put(DataBaseScheme.PlantsTable.Cols.SPECIES, plant.getSpecies());
        values.put(DataBaseScheme.PlantsTable.Cols.DEFAULT_WATERING, plant.getDefaultWatering());
        values.put(DataBaseScheme.PlantsTable.Cols.DAY_FOR_WATERING, serialize(plant.getDayForWatering())); // следующий день полива
        values.put(DataBaseScheme.PlantsTable.Cols.IS_DEFAULT, plant.isDefault());
    ...
    public static byte[] serialize(Object obj) { //Чтобы записать объект Java в БД как byte[];
    ...
    private static ContentValues getSpeciesContentValues(Species species){
        ...
        values.put(DataBaseScheme.SpeciesTable.Cols.SPECIES, species.getSpecies().toString());
        values.put(DataBaseScheme.SpeciesTable.Cols.DEFAULT_WATERING, species.getDefaultWateringInterval());
        return values;
    }
    ...
    public static Species getSpecies(Context context, Integer species_id){
        DataBaseCursorWrapper cursor = queryDbSpecies(context, DataBaseScheme.SpeciesTable.Cols.SPECIES + " = ?",
                new String[]{species_id.toString()});
        try {
            cursor.moveToFirst();
